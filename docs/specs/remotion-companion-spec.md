...
- `--fps` — effective fps. For video, drives extraction; for a frames-dir, sets `source.fps_effective` and the loop frame duration (`round(1000/fps)` ms).
...
- **G8 — loop.webp structure + fps binding** (NEW): `loop.webp` exists and is a real **animated** WebP — RIFF/WEBP, a `VP8X` chunk with the `ANIM` flag, an `ANIM` chunk, and an `ANMF` frame-chunk count **equal to `frames.count`**. Additionally, for **every** ANMF chunk, read the 24-bit little-endian **Frame Duration** field (ANMF payload offset 12) and assert `duration_ms == round(1000 / manifest.loop.fps)`. This **binds fps to the baked bytes**: a manifest-fps-only edit fails here even though the bytes (and G9) are untouched. Parsed by walking RIFF chunks with `node:fs` (zero-dep). FAIL ⇒ non-zero.
...
Together: re-encoding at a different fps changes the bytes (G9 fails) and the durations (G8 fails); editing only `manifest.loop.fps` leaves G9 green but fails G8's duration check. The gap §6.4 names is fully closed.
...
`backend/app/loop_export.py` builds `loop.webp` from the contiguous WebP frame sequence using **Pillow** `Image.save(out, save_all=True, append_images=[...], duration=round(1000/fps), loop=0)`. **ffmpeg `-c:v libwebp_anim`** (already a system prerequisite) is the documented fallback selected per §6.9. The Node kernel stays zero-dependency and never encodes. Export runs in `build_and_verify` (loop mode, post fail-fast); its path is handed to the Node builder via `--loop-webp`.
...
### 6.9 Phase-0 empirical check before cementing G8

G8 asserts `ANMF count == frames.count` **and** per-frame `duration == round(1000/fps)`. The research empirically confirmed one-ANMF-per-frame for `ffmpeg libwebp_anim` and `img2webp` but **not** for Pillow `save_all`. **Phase 0 must verify, on real frames, that the chosen encoder (a) emits exactly one ANMF per input frame with no dedup/merge and (b) writes the exact per-frame duration we pass.** If Pillow dedups or rewrites durations, switch the default primitive to ffmpeg `libwebp_anim`. This check gates whether G8 is even definable and is a Phase-0 exit criterion (§12). It is the one §15 item that can block Phase 0.
...
- `loop_export`: **one-ANMF-per-frame on real frames** (§6.9), exact per-frame duration, sha stability.
- Kernel self-test: scroll **golden-package byte-diff** (every output file, excl. `id`/`created_at`; pinned fingerprint within it — §13); loop build→verify pass; corrupt frame / `webp_sha256` / ANMF-duration / ANMF-count each fail the matching gate.
...
**Phase 0 — loop kernel (one workflow → then SEE it).** **Do the encoder spike FIRST (§6.9):** on real frames, confirm the chosen encoder emits exactly one ANMF per frame with the exact per-frame duration — *before* building G8 around it. If Pillow dedups or rewrites durations, switch the default to ffmpeg `libwebp_anim` and design G8 against the real bytes. Then freeze `loop.v1`: template + `MODE_CONFIG`/`SCHEMA_CONFIG` dispatch + loop branch (G8/G9, adapted G4, loop README) + `CONTRACT-loop.md` + `loop_export.py` + the reference loop player. **Gate with negative-corruption tests for both templates** (corrupt a frame, the sha, an ANMF duration, the ANMF count ⇒ the matching gate fails) **and a scroll golden-package byte-diff** (byte-identical output excl. per-build `id`/`created_at`; any scroll drift fails). Then **open the loop player in a browser (chrome-devtools MCP)** and confirm it loops with **zero external requests**. Exit criteria: spike resolved, all gated, golden green, *and* seen looping offline.
...
- **`loop.webp` encoder (Pillow vs ffmpeg `libwebp_anim`)** — provisional until the **Phase-0 §6.9 check** (one ANMF per frame + exact per-frame duration) passes. **This is the only §15 item that can block Phase 0**; if Pillow dedups or rewrites durations, switch the default to ffmpeg.
